...
The Installation Client (`MPTClient`) is an authenticated HTTP client that operates in the scope of a specific extension installation (account).

## What is an Installation?

An **installation** represents a specific account's instance of your extension. When a customer installs your extension in their Marketplace account, a unique installation is created. Each installation has:
...
1. **Scoped Authentication**: All API calls are authenticated in the context of a specific installation
2. **Automatic Token Management**: Handles token acquisition, caching, and renewal automatically
...
async def my_endpoint(ctx: AuthContext, client: InstallationClient):
    # Client is already configured for ctx.installation_id
...
async def process_data(installation_id: str):
